...
# Arma el listado de personas_inversoras (no están catalogades como inversores ángel)
list_personas_inversoras <- inversores_a_argentina |> 
  filter(investor_type %in% c("Individual/Angel", "Individual/Angel, Investment Partner")) |> 
  pull(`organization/person_name`)

list_inversores_a_argentina <- inversores_a_argentina |> 
  pull(`organization/person_name`)
...
inversiones_argentina <- inversiones_argentina |> 
  filter(!investor %in% list_personas_inversoras)

# Incluimos sólo a quienes a los inversores que están incluidos en el listado de inversores a Argentina
inversiones_argentina <- inversiones_argentina |> 
  filter(investor %in% list_inversores_a_argentina)

# Elimino los objetos que ya no necesito
rm(list_personas_inversoras, list_inversores_a_argentina)
